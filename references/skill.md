...
3. **No real names or companies**: Replace everything with vague descriptions
...
```

```
...
- **AI Taming**: Does the user effectively and precisely get AI to complete tasks? CLIENT and HIRE score high, SPOON and IDC score low
- **Social Death Risk**: How embarrassing would it be if chats were public? VENT and EMO are high risk (S/A), CORP is low risk (D)
- **Gold Content**: Does the conversation produce real output? NERD and HIRE score high, LOOP and IDC score low
- **Mental State**: Look for emotional dumping, late-night sessions, self-doubt. EMO and VENT get red, CORP gets green
- **AI Dependency**: How dependent is the user on AI? HIRE and SIMP score high, CORP and DRY score low. One hidden type gets max score
...

{
  ...
  "indices": {
    ...
    "detail": {
      ...
      "gold": "Brief explanation (max 50 chars)",
...
- `indices.tame` / `gold` / `depend`: 0-100 integers
...
- `indices` is a single-line JSON object. `tame` / `gold` / `depend` are 0-100 integers. `risk` is one of S/A/B/C/D. `mental` is one of green/yellow/red.
...
{"tame":75,"risk":"A","gold":60,"mental":"yellow","depend":80,"detail":{"tame":"shitpost-style one-liner, funny and specific","risk":"shitpost-style one-liner","gold":"shitpost-style one-liner","mental":"shitpost-style one-liner","depend":"shitpost-style one-liner"}}
